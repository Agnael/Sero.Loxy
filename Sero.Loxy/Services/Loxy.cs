...
namespace Sero.Loxy
{
        ...
        where TContext : class
    {
        ...
        private readonly Dictionary<Guid, TimestampedEventCandidate> _keyedCandidateMap;
            ...
            IScopeBuilder scopeBuilder)
        {
            ...
            _keyedCandidateMap = 
                new Dictionary<Guid, TimestampedEventCandidate>();
        ...
        public void Raise(IEventCandidate evt)
        {
            ...
            _keyedCandidateMap.Add(Guid.NewGuid(), timestampedEvt);
            ...
            foreach (ISink sink in _sinks)
            {
                if (maxCandidateLevel >= sink.GetMinimumLevel())
                {
                    ...
                    if (context == null)
                    {
                        ...
                        foreach (
                            Action<TContext> contextConfigurator 
                            in _contextConfigurators)
                        {
                    ...
                    IEnumerable<IEvent> events = 
                        keyedEventMap.Select(x => x.Value);

                    IScope scope = 
                        await _scopeBuilder.Build(context, _contextConfigurators.Count, events);
